...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        "Welcome to PDF Bot\n\n"
        "- Send images then type /pdf to convert\n"
        "- Send PDF files then type /merge to join them\n"
        "- Type /clear to delete your files"
    )
...
async def handle_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Image saved ({len(data['images'])}). Send more or type /pdf")
...
async def handle_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"PDF saved ({len(data['pdfs'])}). Send more or type /merge")
...
async def generate_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        images = [Image.open(p).convert("RGB") for p in data["images"] if os.path.exists(p)]
        if images:
        ...
        await update.message.reply_text("Error during conversion")

async def merge_files(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    data = user_data[uid]
    if len(data["pdfs"]) < 2:
        await update.message.reply_text("Send at least 2 PDFs")
        return
    ...
    try:
        merger = PdfMerger()
        for p in data["pdfs"]:
            if os.path.exists(p): merger.append(p)
        merger.write(output)
        merger.close()
        with open(output, "rb") as f:
        ...
        await update.message.reply_text("Error during merge")
